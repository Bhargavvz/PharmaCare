...
// Define interfaces for pharmacy-related DTOs
...
export interface InventoryOverviewDataPoint {
  category: string;
  value: number;
}
...
export const pharmacyService = {
  ...
  // Add staff related operations
  getPharmacyStaff: async (pharmacyId: number): Promise<any[]> => {
    const response = await api.get(`/pharmacies/${pharmacyId}/staff`);
    return response.data;
  },
  
  addPharmacyStaff: async (pharmacyId: number, staffData: {
    email: string;
    firstName: string;
    lastName: string;
    role: string;
    ...
    const response = await api.post(`/pharmacies/${pharmacyId}/staff`, staffData);
    return response.data;
  },
  
  updatePharmacyStaff: async (pharmacyId: number, staffId: number, staffData: {
    firstName?: string;
    lastName?: string;
    role?: string;
    active?: boolean;
  }): Promise<any> => {
    const response = await api.put(`/pharmacies/${pharmacyId}/staff/${staffId}`, staffData);
    return response.data;
  },
  
  deletePharmacyStaff: async (pharmacyId: number, staffId: number): Promise<any> => {
    const response = await api.delete(`/pharmacies/${pharmacyId}/staff/${staffId}`);
    return response.data;
  }
};
...
export const inventoryService = {
  ...
  getInventoryItems: async (pharmacyId: number, page: number = 0, size: number = 20, search?: string): Promise<{
    content: InventoryItemDto[],
    totalPages: number,
    totalElements: number
  }> => {
    try {
      let url = `/inventories/${pharmacyId}/items?page=${page}&size=${size}`;
      if (search) {
      ...
      console.error(`Error fetching inventory items for pharmacy ${pharmacyId}:`, error);
      throw error;
    }
  },

  // Get a specific inventory item
  getInventoryItem: async (pharmacyId: number, itemId: number): Promise<InventoryItemDto> => {
    try {
      const response = await api.get(`/inventories/${pharmacyId}/items/${itemId}`);
      return response.data;
    } catch (error) {
      console.error(`Error fetching inventory item ${itemId} for pharmacy ${pharmacyId}:`, error);
      throw error;
    }
  },

  // Add a new inventory item
  addInventoryItem: async (pharmacyId: number, itemData: {
    medicationName: string;
    batchNumber: string;
    manufacturer?: string;
    quantity: number;
    ...
    try {
      const response = await api.post(`/inventories/${pharmacyId}/items`, itemData);
      return response.data;
    } catch (error) {
      console.error(`Error adding inventory item to pharmacy ${pharmacyId}:`, error);
      throw error;
    }
  },

  // Update an inventory item
  updateInventoryItem: async (pharmacyId: number, itemId: number, itemData: Partial<InventoryItemDto>): Promise<InventoryItemDto> => {
    try {
      const response = await api.put(`/inventories/${pharmacyId}/items/${itemId}`, itemData);
      return response.data;
    } catch (error) {
      console.error(`Error updating inventory item ${itemId} for pharmacy ${pharmacyId}:`, error);
      throw error;
    }
  },

  // Delete an inventory item
  deleteInventoryItem: async (pharmacyId: number, itemId: number): Promise<void> => {
    try {
      await api.delete(`/inventories/${pharmacyId}/items/${itemId}`);
    } catch (error) {
      console.error(`Error deleting inventory item ${itemId} from pharmacy ${pharmacyId}:`, error);
      throw error;
    }
  },
  
  // Search inventory items
  searchInventoryItems: async (pharmacyId: number, searchTerm: string): Promise<InventoryItemDto[]> => {
    try {
      const response = await api.get(`/inventories/${pharmacyId}/items/search?term=${encodeURIComponent(searchTerm)}`);
      return response.data;
    } catch (error) {
      console.error(`Error searching inventory items for pharmacy ${pharmacyId}:`, error);
      throw error;
    }
  }
};
...
export const billingService = {
  ...
  getBills: async (pharmacyId: number, page: number = 0, size: number = 20, search?: string): Promise<{
    content: BillDto[],
    totalPages: number,
    totalElements: number
  }> => {
    try {
      let url = `/pharmacies/${pharmacyId}/bills?page=${page}&size=${size}`;
      if (search) {
        url += `&search=${encodeURIComponent(search)}`;
      }
      ...
      console.error(`Error fetching bills for pharmacy ${pharmacyId}:`, error);
      throw error;
    }
  },

  // Get a specific bill
  getBill: async (billId: number): Promise<BillDto> => {
    try {
  ...
  updateBillStatus: async (billId: number, paymentStatus: string): Promise<BillDto> => {
    try {
      const response = await api.patch(`/bills/${billId}/status`, { paymentStatus });
      return response.data;
    } catch (error) {
      console.error(`Error updating payment status for bill ${billId}:`, error);
      throw error;
    }
  },
  
  // Get bill summary/statistics
  getBillStats: async (pharmacyId: number, period: string): Promise<any> => {
    try {
      const response = await api.get(`/pharmacies/${pharmacyId}/bills/stats?period=${period}`);
      return response.data;
    } catch (error) {
      console.error(`Error fetching bill stats for pharmacy ${pharmacyId}:`, error);
      throw error;
    }
  }
};
